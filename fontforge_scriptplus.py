...
ADD_GLYPH_EN = settings.get("DEFAULT", "ADD_GLYPH_EN")
ADD_GLYPH_JP = settings.get("DEFAULT", "ADD_GLYPH_JP")
...
def generate_font(jp_style, eng_style, merged_style):
    ...
    # 本来のUDEV GOTHICに無い、勝手に追加した要素を組み込む。
    if options.get("ysmod"):
        add_glyph_jp(jp_font, jp_style)
        add_glyph_en(eng_font, eng_style)
...
def open_fonts(jp_style: str, eng_style: str):
    ...
    add_glyph_jp = fontforge.open(
        SOURCE_FONTS_DIR + "/" + ADD_GLYPH_JP.replace("{style}", jp_style)
    )
    add_glyph_en = fontforge.open(
        SOURCE_FONTS_DIR + "/" + ADD_GLYPH_EN.replace("{style}", eng_style)
    )
    if options.get("liga"):
...
"""UDEVゴシックに追加した要素このへん"""
def add_glyph_jp(jp_font, jp_style: str):
    # 全角長音を差し替え
    glyph = jp_font[0x30FC]
    glyph.clear()
    jp_font.mergeFonts(fontforge.open(f"{SOURCE_FONTS_DIR}/{ADD_GLYPH_JP.replace('{style}', jp_style)}"))
    jp_font.selection.none()

def add_glyph_en(eng_font, eng_style: str):
    # Dとパイプを差し替え
    glyph = eng_font[0x0044]
    glyph.clear()
    glyph = eng_font[0x007C]
    glyph.clear()
    eng_font.mergeFonts(fontforge.open(f"{SOURCE_FONTS_DIR}/{ADD_GLYPH_EN.replace('{style}', eng_style)}"))
    eng_font.selection.none()
